...
function scheduleInterferenceCheck(delay = 150) {
  ...
  _interferenceTimer = setTimeout(() => {
    ...
    if (visibleMeshes.length < 2) {
      wasColliding = false; // Forzamos a olvidar la colisión si no hay modelos suficientes
      return; // Ahora sí, abortamos de forma segura
    }
    // -------------------------------------
...
// Eliminamos límites para giro total
controls.minPolarAngle = -Infinity; // Permite dar la vuelta completa vertical
controls.maxPolarAngle = Infinity;
controls.minAzimuthAngle = -Infinity; // Giro horizontal infinito
controls.maxAzimuthAngle = Infinity;
// Por defecto el giro horizontal (Azimuth) ya es infinito en OrbitControls
...
// ---------------- ILUMINACIÓN PLANA TIPO ESCÁNER ----------------
// 1. Luz Ambiental alta: Ilumina el 100% del modelo por igual, eliminando sombras oscuras.
const ambientLight = new THREE.AmbientLight(0xffffff, 0.55); 
scene.add(ambientLight);

// 2. Luz de Cámara suave: Un pequeño refuerzo frontal para dar volumen sin crear brillos.
const cameraLight = new THREE.DirectionalLight(0xffffff, 0.65);
...
const COLOR_PRESETS = [
  { name: 'Original', hex: 0xc0c5ce }, { name: 'Blanco', hex: 0xffffff }, { name: 'Amarillo', hex: 0xffd400 },
...
function setMeshColor(mesh, hex) {
  if (!mesh?.material) return
  if (mesh.userData.baseColorHex == null) mesh.userData.baseColorHex = mesh.material.color?.getHex?.() ?? 0x9aa0a6
...
function checkInterferences() {
  ...
  if (meshes.length < 2) {
    wasColliding = false; // Reseteamos la memoria de colisiones si no hay modelos suficientes
...
function visualizeInterferencePoints(points) {
  ...
  // MATERIAL CORREGIDO PARA AURA 3D
  const material = new THREE.PointsMaterial({
    color: 0xff0000, // Rojo puro intenso
    size: 6.0,       // Tamaño ligeramente mayor para que destaque
    map: collisionDotTexture, 
    transparent: true,
    opacity: 0.9,
    depthTest: false, // Siempre visibles por encima del modelo
    sizeAttenuation: true,
    blending: THREE.NormalBlending // Importante: evita que se laven con la luz blanca
  })
...
function createRow(mesh) {
  ...
  row.innerHTML = `
    ...
    <div class="colorDropdown">
      <button class="btnColor" type="button" data-action="color-dropdown">
        <span class="colorIndicator" data-color-indicator style="background:#9aa0a6"></span>
        Color Material
      </button>
  ...
  row.querySelector('[data-action="delete"]').addEventListener('click', () => {
    ...
    modelsGroup.remove(mesh); 
    mesh.geometry?.dispose?.(); 
    mesh.material?.dispose?.(); 
    rowById.delete(mesh.userData.id); 
    row.remove(); 
    setCount();
    
    // -- NUEVA LÍNEA --
    // Obligamos al motor a recalcular (y resetear) al borrar una pieza
...
async function loadModelFile(file) {
  ...
  try {
    ...
    // Usamos MeshLambertMaterial para un acabado mate perfecto sin brillos
    const mesh = new THREE.Mesh(geometry, new THREE.MeshStandardMaterial({ 
        color: 0xc0c5ce,     // Un gris neutro muy profesional que descansa la vista
        roughness: 0.50,     // Lo suficientemente liso para tener brillos en las cúspides
        metalness: 0.0,     // Muy bajito para que parezca resina/plástico, no metal
        side: THREE.DoubleSide
    }))
    mesh.userData.id = `m${idCounter++}`; mesh.userData.name = file.name; mesh.userData.opacity = 1; mesh.userData.colorHex = 0x9aa0a6; // Guardamos el gris en la memoria del modelo
...
btnScreenshot.addEventListener('click', () => {
  ...
  link.download = `Aura3D_Captura_${new Date().toISOString().slice(0,10)}.png`
...
// ---------------- EXPORTAR SIEMPRE A STL (DESCARGA DIRECTA BINARIA OPTIMIZADA) ----------------
const stlExporter = new STLExporter();

btnSave.addEventListener('click', async () => {
  if (!selectedMesh) {
    showCollisionToast("⚠️ Selecciona un modelo de la lista primero", 3000);
    return;
  }

  // Cogemos el nombre original y le ponemos siempre la extensión .stl
  const originalName = selectedMesh.userData.name || 'modelo_aura3d';
  const baseName = originalName.replace(/\.[^/.]+$/, "");
  const finalName = `${baseName}_modificado.stl`;

  try {
    // 1. Clonamos la geometría para prepararla
    selectedMesh.updateMatrixWorld(true);
    let exportGeo = selectedMesh.geometry.clone();
    
    // 2. Aplicamos la rotación/posición actual (vital si usaste el botón "Alinear")
    exportGeo.applyMatrix4(selectedMesh.matrixWorld);
    
    // 3. LA MAGIA: Los STL exigen triángulos sueltos. Si es un OBJ/PLY, lo aplanamos.
    if (exportGeo.index) {
      exportGeo = exportGeo.toNonIndexed();
    }

    // 4. Creamos una malla temporal y la preparamos para el exportador
    const tempMesh = new THREE.Mesh(exportGeo, new THREE.MeshBasicMaterial());
    tempMesh.updateMatrixWorld(true);

    // 5. GENERAMOS EL STL EN MODO BINARIO (¡Esto soluciona el problema del peso gigante!)
    const stlData = stlExporter.parse(tempMesh, { binary: true });
    if (!stlData) throw new Error("Fallo al generar la geometría STL");

    // Usamos 'application/octet-stream' porque ahora es un archivo binario comprimido
    const blob = new Blob([stlData], { type: 'application/octet-stream' });

    // 6. DESCARGA DIRECTA SECRETA (Esquivamos el bloqueo de tu programa .exe)
    const link = document.createElement('a');
    ...
    showCollisionToast("✅ Guardado STL optimizado con éxito", 3000);

  } catch (error) {
    console.error("Error al exportar a STL:", error);
    showCollisionToast(`❌ Error: ${error.message}`, 5000);
  }
});
...
function startRecording() {
    recordedChunks = [];
    // Capturamos el stream del canvas de Three.js a 60 FPS
    const stream = renderer.domElement.captureStream(60); 
    
    // Intentamos usar el codec H.264 para máxima compatibilidad con MP4
    ...
    mediaRecorder.onstop = () => {
        ...
        a.href = url;
        // Nombre del archivo con la marca de Lumix
        a.download = `Aura3D_Video_${new Date().getTime()}.mp4`;
        a.click();
